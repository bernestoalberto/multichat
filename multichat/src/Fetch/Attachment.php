...
class Attachment
{
    ...
    public function Construir(Message $message, $partIdentifier = null)
    {
        $this->messageId  = $message->getUid();
        $this->imapStream = $message->getImapBox()->getImapStream();
        $this->structure  = $message->getStructure();

        if (isset($partIdentifier))
            $this->partId = $partIdentifier;

        $attachments = $message->getAttachments();
        $this->size = $attachments[0]->size;
        $this->setFileName($attachments[0]->filename);

        $this->mimeType = Message::typeIdToString($this->mimeType);


        $this->encoding = $attachments[0]->encoding;
    }
    ...
    public function getMyData()
    {
        if (!isset($this->data)) {
            ...
            $messageBody  = imap_fetchbody($this->imapStream, $this->messageId, 2 ?: 1, FT_UID);
 /*           $messageBody = isset($this->partId) ?
                imap_fetchbody($this->imapStream, $this->messageId, $this->partId, FT_UID)
                : imap_body($this->imapStream, $this->messageId, FT_UID);*/

            $messageBody = Message::decode($messageBody,3);
            $this->data  = $messageBody;
        }
